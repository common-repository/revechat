...
class WP_Plugin_Revechat
{
    ...
    public function getCartInfo()
    {
        ...
        $customer=wp_set_current_user( $user_id );
        
        
        WC()->customer = new WC_Customer( $customer, true );
        WC()->cart = new WC_Cart();
        
        $response = new WP_REST_Response($this->getCartPayload($customer), 200);
     ...
     *
     */
    public function getCartPayload($currentCustomer)
    {
        $products = array();
        ...
        if (isset($currentCustomer) && isset($currentCustomer->ID) )
        {
    ...
    {
        // Add JS.
        wp_enqueue_script('revechat', plugin_dir_url(__FILE__) . 'assets/js/woocommerceCart.js?ver='.round(microtime(true)*1000), ['jquery'], NULL, TRUE);
        // Pass nonce to JS.
